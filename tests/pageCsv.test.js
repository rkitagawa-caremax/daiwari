...
test('buildPageCsvRowsForSheet numbers visible panels and skips hidden ones', () => {
  ...
  // 先頭コマ: 追番1 / コマ番号1 / 座標 X1Y1
  assert.equal(rows[0], '食事関連,3,1,1,A1234,1/8 横（2コマ）,,,X1Y1,,1,1');
  // 特殊ダミー(タイトル)は追番は進むがコマ番号は空欄、コードは「ダミーコマ」
  assert.equal(rows[1], '食事関連,3,,2,ダミーコマ,1/16（1コマ）,,,X3Y1,,3,1');
  // テキスト情報はカンマを含むのでダブルクォートで囲まれる
  assert.equal(rows[4], '食事関連,3,4,5,,1/16（1コマ）,,"テキスト, 付き",X2Y2,,2,2');
});
